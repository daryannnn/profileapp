...
export default async function searchOrganizations(name: string, interest: string[], programs: boolean, events: boolean) {
    let usersIds: Array<string> = [];

    const querySnapshotUsers = await getDocs(query(collection(db, "Users"), where("userType", "==", "organization")));
    ...
    if (interest.length > 0) {
        ...
        const querySnapshot = await getDocs(query(collection(db, "Users"), where("userType", "==", "organization")));
